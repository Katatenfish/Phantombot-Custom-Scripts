...
(function() {
    var //set the variables needed 
        waitTime = $.getSetIniDbNumber('tamagotchi_BattleSettings', 'waitTime', 60) * 1000,
        foodCost = $.getSetIniDbNumber('tamagotchi_BattleSettings', 'foodCost', 2),
        lostFunDecr = $.getSetIniDbNumber('tamagotchi_BattleSettings', 'lostFunDecr', 2),
        winFunIncr = $.getSetIniDbNumber('tamagotchi_BattleSettings', 'winFunIncr', 4),
        baseGainExp = 0.75,
    ...
    /**
     * @function reloadBattleSettings
     */
    function reloadBattleSettings() {
        waitTime = $.getIniDbNumber('tamagotchi_BattleSettings', 'waitTime'),
        foodCost = $.getIniDbNumber('tamagotchi_HugSettings', 'foodCost'),
        lostFunDecr = $.getIniDbNumber('tamagotchi_HugSettings', 'lostFunDecr'),
        winFunIncr = $.getIniDbNumber('tamagotchi_HugSettings', 'winFunIncr');
    }
    ...
    function prepareBattle(sender, target) {
        ...
        currentBattle.waitTimerId = setTimeout(function() {
            $.say($.lang.get('tgbattle.wait.timedout', $.whisperPrefix(targetTG.owner)));
        ...
        $.say($.lang.get('tgbattle.wait.foraccept', senderTG.name, targetTG.name, $.username.resolve(targetTG.owner)));
    }
    ...
    $.bind('command', function(event) {
        ...
        if (command.equalsIgnoreCase('reloadbattlesettings')) {
            reloadBattleSettings();
        }
    });
    ...
    $.bind('initReady', function() {
        if($.bot.isModuleEnabled('./custom/games/tamagotchi/tg-battle.js')){
            $.registerChatCommand('./custom/games/tamagotchi/tg-battle.js', 'tgbattle', 6);
            $.registerChatCommand('./custom/games/tamagotchi/tg-battle.js', 'tgbaccept', 6);

            $.registerChatCommand('./custom/games/tamagotchi/tg-battle.js', 'reloadbattlesettings', 30);
        }
    });
})();